      ...
      while [ true ]; do
        if [[ $1 == -* ]]; then
          ...
          --template)
            ...
            ;;
          --git-branch)
            dic[opt_git_branch]=$2
            shift 2
            ;;
          --build-cmds)
            dic[opt_build_cmds]=$2
            shift 2
            ;;
          --build-env)
            dic[opt_build_env]=$2
          ...
          --workspace)
            ...
            ;;
          *)
            error "unknown parameter or command $1 ."
            exit 1
            break
            ;;
          esac
...
function run() {
  ...
  run)
    if test -n ${dic[cmd_2]}; then
      ...
      echo "run need be followed by a cammand"
      exit 1
    fi
    ;;
  *)
    error "cannot find the cammand ${dic[cmd_1]}"
    exit 1
    ;;
  esac
}
...
function check_post_parmas() {
  if [[ -z ${dic[cmd_3]} ]]; then
    warn "job name can not be null ## $1 ##."
    exit 1
  fi
...
function java_build() {
  ...
  maven)
    ...
    ;;
  *)
    warn "java project only support gradle or maven build"
    exit 1
    ;;
  esac
  ...
  echo "${dic[tmp_build_dist_path]}"
  echo "${cmd_job_name}"
  jar_name=$(ls | grep -v 'source' | grep "^${cmd_job_name}.*\.jar$")
  info "jar_name：${jar_name}"
  check_env_by_cmd_v docker
  # 构建镜像
  image_path=$cfg_docker_registry_address/$cfg_docker_registry_root/${cmd_job_name}:${tmp_docker_image_suffix}
...
function choose_dockerfile() {
  ...
  if test -n "${opt_dockerfile}"; then
    ...
    for dockerfile in ${dockerfiles[@]}; do
      if [[ $cmd_job_name == $dockerfile ]]; then
...
function local_deploy() {
  ...
  elif [ "$cfg_build_platform" = "DOCKER_SWARM" ]; then
...
function prune() {
  ...
  if test -n "$image_num"; then
    if test $image_num -gt 30; then
      ...
      sudo docker image prune -af --filter="label=maintainer=corp"
    fi
  fi
}
